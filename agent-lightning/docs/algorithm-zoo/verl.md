...
PrefixGrouper currently requires `use_remove_padding=False`, fused kernels disabled, Ulysses sequence parallel size 1, and text-only batches. CUDA supports FSDP/FSDP2; Ascend uses FSDP because its startup path performs layer-at-a-time sharded checkpoint loading. Unsupported multimodal, distillation-top-k, and sum-pi-squared batches fall back to the standard VERL forward.

Agent Lightning calls VERL's device auto-configuration before creating Ray resources. If `torch-npu` and a usable Ascend device are present it selects `trainer.device=npu`; otherwise it selects CUDA. `VERL_PLATFORM=huawei` or `VERL_PLATFORM=nvidia` remains available as an explicit override. After Ray starts in NPU mode, Agent Lightning discovers every live node's `NPU` resource and replaces the static `trainer.nnodes` and `trainer.n_gpus_per_node` values with the homogeneous topology visible to Ray. The preflight uses VERL's actual per-replica footprint, `TP * DP * PP`, including the larger prefill/decode footprint when disaggregation is enabled. It requires the full world size and per-node slicing to contain complete replicas, validates the reward-model rollout separately, and reports all parallel dimensions and replica counts. Configurations that would leave an idle rank or split a sub-node replica across two nodes fail before model loading. Limit an NPU job by restricting `ASCEND_RT_VISIBLE_DEVICES` before starting Ray; otherwise all NPU resources registered in the Ray cluster are used.

When NPU is selected, every enabled actor/reference, critic, and reward-model ModelScope ID is automatically downloaded before Ray starts. A repository is stored below the directory where the command was launched, shared IDs are downloaded once, and every corresponding VERL path is rewritten to the same absolute local directory. For example, launching with `actor_rollout_ref.model.path=Qwen/Qwen2.5-0.5B-Instruct` creates `./Qwen--Qwen2.5-0.5B-Instruct/`. Existing local model directories are used directly. Explicit `hf_config_path`, `tokenizer_path`, and `lora_adapter_path` repositories are materialized in the same way.

Ascend actor, reference, and critic workers build a meta model, balance safetensors parameters across rank-local CPU memory, and materialize one FSDP-wrapped layer at a time. Both VERL `language_model` and `value_model` engines are covered, whether or not PrefixGrouper is enabled. This avoids VERL 0.9's default rank-0 peak, where the complete checkpoint is moved to one NPU before FSDP shards it. Agent Lightning fixes each FSDP shard group to the complete NPU world and requires `actor_rollout_ref.actor.strategy=fsdp`; an enabled critic must also use `critic.strategy=fsdp`. The path requires a safetensors checkpoint and full-parameter training; LoRA and QAT are rejected during startup rather than silently falling back to the rank-0 loader.

The colocated NPU actor-to-vLLM update uses VERL's `naive` checkpoint engine so every actor rank updates its colocated rollout rank. Agent Lightning rejects other checkpoint backends in this path: the pinned HCCL implementation has only actor rank 0 transmit full weights and therefore reintroduces a single-rank communication and memory hotspot. Set `actor_rollout_ref.rollout.checkpoint_engine.backend=naive` explicitly in saved production configurations.
...
Use `--backend gpu` or `--backend npu` to override installation detection, and `--dry-run` to inspect the commands without changing the environment. The runtime benchmark accepts `--device auto` (the default), `gpu`/`cuda`, `npu`, or an indexed device such as `npu:1`. An unindexed device automatically relaunches through torchrun on every visible local device, using loopback rendezvous so it also works on hosts whose external interface cannot be reached locally. Use `--nproc-per-node N` to limit the number of devices or an indexed device/`--nproc-per-node 1` for an intentional single-device run. Multi-device benchmark models use VERL-compatible FSDP `FULL_SHARD`: rank 0 stages real CPU weights, other ranks build meta parameters, and FSDP synchronizes and shards the model without first placing a complete copy on every accelerator. Each rank uses a distinct deterministic input batch. Production actor/reference/critic loading uses the same FSDP sharding model, while vLLM rollout uses the configured TP/DP/PP topology.

On NPU, whole-device power sampling is enabled by default and uses `npu-smi info -t power -i DEVICE -c CHIP` on every rank. Use `--no-power` to disable it or `--power` to enable the corresponding `nvidia-smi` sampling on GPU. Multi-device reports use the slowest rank as step latency, sum response tokens and whole-device power across ranks, and include per-rank plus maximum/mean/summed memory. Benchmark control, barriers, and numerical-statistic aggregation run through a dedicated CPU Gloo group; HCCL/NCCL is reserved for FSDP parameter all-gather and gradient reduce-scatter. PrefixGrouper supplies its padding masks inside the patched attention function, avoiding the redundant Transformers model-level mask reduction and NPU scalar read. The JSON and Markdown reports include p50/p90/p95/p99 latency, variation, throughput, theoretical dense-token and causal-pair reduction, peak and incremental memory, idle/load power, estimated joules per step, tokens per joule, and detailed per-rank numerical agreement. Power is sampled in a separate synchronized workload so it does not contaminate the latency samples. Runtime PPA is defined as Performance, Power, and Accuracy; physical chip Area is not measurable by this script and is not fabricated from a proxy.
