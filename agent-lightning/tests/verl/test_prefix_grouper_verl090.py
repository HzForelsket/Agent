...
def test_native_npu_attention_uses_bnsd_and_full_mask(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[dict] = []
    ...
    prefix_key = torch.randn(2, 4, 3, 8)
    prefix_value = torch.randn(2, 4, 3, 8)
    prefix_attention_mask = prefix_grouper_module._causal_sdpa_mask(grouper.prefix_attn_mask, 3, 3)
    prefix_output = prefix_grouper_module._npu_bnsd_attention(
        prefix_query,
        prefix_key,
        prefix_value,
    ...
    suffix_key = torch.randn(4, 4, 5, 8)
    suffix_value = torch.randn(4, 4, 5, 8)
    suffix_attention_mask = prefix_grouper_module._causal_sdpa_mask(grouper.suffix_attn_mask, 2, 5)
    suffix_output = prefix_grouper_module._npu_bnsd_attention(
        suffix_query,
        suffix_key,
        suffix_value,
    ...
    assert prefix_call["input_layout"] == suffix_call["input_layout"] == "BNSD"
    assert prefix_call["query"].shape == (2, 4, 3, 8)
    assert prefix_call["key"].shape == (2, 4, 3, 8)
    assert prefix_call["keep_prob"] == 1.0
    assert prefix_call["sparse_mode"] == 0

    assert suffix_call["query"].shape == (4, 4, 2, 8)
    assert suffix_call["key"].shape == (4, 4, 5, 8)
    assert suffix_call["keep_prob"] == pytest.approx(0.9)
    assert suffix_call["scale"] == 0.125

    prefix_attention_mask = prefix_call["atten_mask"]
    assert prefix_attention_mask.shape == (2, 1, 3, 3)
    assert prefix_attention_mask.dtype == torch.bool
    assert not prefix_attention_mask[0, 0, 0, 0]
    assert prefix_attention_mask[0, 0, 0, 1]
    assert prefix_attention_mask[1, 0, 0].all()

    suffix_attention_mask = suffix_call["atten_mask"]
    assert suffix_attention_mask.shape == (4, 1, 2, 5)
    assert not suffix_attention_mask[0, 0, 0, 0]
    assert suffix_attention_mask[0, 0, 0, 4]
